...
double apply_discount(double before_discount, double discount) {
    ...
    double newDisc = before_discount * ((100 - discount ) / 100);
    return newDisc;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double discountPrice = apply_discount(hourly_rate, discount) * 22;
    

    return std::ceil(discountPrice);
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double newPrice = budget * (100 - (discount/100));
    double totalHours = newPrice / hourly_rate;
    return totalHours / 8;
}